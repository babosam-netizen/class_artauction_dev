...
export function AdminGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!unlocked) adminPasswordExists().then(setExists);
  }, [unlocked]);
  ...
  async function handleSubmit() {
    ...
    try {
      if (exists === false) {
        if (pw.length < 6) return setError('관리자 암호는 6자 이상으로 정해주세요');
