...
export function GalleryView({
  ...
  }, [current?.id, code, studentNumber, prompts.length]);
  ...
  async function handleSave() {
    ...
    setBusy(true);
    try {
      await saveAppreciation(code, studentNumber, current.id, answers);
      setSaved(true);
      setOpen(false);
  ...
  return (
    <Wall>
      {/* 작품 영역 */}
      <div
        className="relative z-[2] flex flex-1 flex-col items-center justify-center transition-all"
        style={{ transition: 'all 0.4s ease' }}
      >
        {/* 화살표 */}
        {index > 0 && (
          <Arrow side="left" onClick={() => setIndex((i) => Math.max(0, i - 1))} />
        )}
        {index < total - 1 && (
        ...
        >
          ...
          <div className="mt-4 text-center" style={{ animation: 'fadeUp 0.5s ease 0.1s both' }}>
            <div
              ...
              style={{ color: C.cream, letterSpacing: '0.5px' }}
            >
              {current.title}
            </div>
            ...
            <button
              ...
              style={{
                ...
                background: 'rgba(196,167,90,0.07)',
                color: C.creamDim,
              }}
            >
              <span>✏️</span>
              <span>{open ? '닫기' : saved ? '감상 다시 보기' : '감상 기록하기'}</span>
            </button>
            {saved && !open && (
              <button
      ...
      {open && (
        <div
          ...
          style={{
            background: C.panel,
            borderLeft: `1px solid rgba(196,167,90,0.18)`,
        ...
        >
          ...
          <div className="px-6 pb-2 pt-5">
            ...
            >
              💬 감상 질문 · {gradeBand === '3-4' ? '3~4학년' : '5~6학년'} · {step + 1}/{prompts.length}
            </div>
          ...
          <div className="flex flex-1 flex-col gap-3 px-6 py-4">
            <textarea
              ...
              style={{
                ...
                borderColor: 'rgba(196,167,90,0.2)',
                color: C.creamDim,
              }}
            />
                ...
                {prompts.map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 3,
                      cursor: 'pointer',
                      background: i === step ? C.gold : 'rgba(196,150,90,0.28)',
                    }}
                  />
                ))}
              </div>
            ...
            <button
              ...
              style={{
                background: 'rgba(196,167,90,0.13)',
                borderColor: 'rgba(196,167,90,0.38)',
                color: C.gold,
              }}
            >
              {busy ? '저장 중…' : '저장하고 해설 보기'}
            </button>
...
function Wall({ children }: { children: React.ReactNode }) {
  return (
    <div
      className="relative flex h-screen w-full overflow-hidden font-body"
...
function Arrow({ side, onClick }: { side: 'left' | 'right'; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="absolute top-1/2 z-[4] flex h-13 w-13 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border"
      style={{
        [side]: 28,
        width: 52,
        height: 52,
...
function CommentaryOverlay({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-lg border p-7"
        style={{ background: C.panel, borderColor: 'rgba(196,167,90,0.25)' }}
      >
        <div className="text-[10px] tracking-[2px]" style={{ color: 'rgba(196,167,90,0.6)' }}>
          작품 해설
        </div>
