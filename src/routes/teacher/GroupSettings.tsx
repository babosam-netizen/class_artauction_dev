    ...
    <div className={bar} style={barStyle}>
      ...
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between">
        <span className="text-sm font-medium" style={{ color: GOLD }}>
          🧩 모둠 설정
          <span className="ml-2 text-xs font-normal" style={{ color: 'rgba(232,217,184,0.6)' }}>
            현재 {meta.groupCount}모둠 · 모둠당 {meta.groupSize ?? 4}명
          </span>
        </span>
        <span style={{ color: GOLD }}>{open ? '▴ 접기' : '▾ 펴기'}</span>
      </button>

      {open && (
        <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-3 border-t pt-3" style={{ borderColor: 'rgba(196,167,90,0.12)' }}>
          <Field label="모둠 수">
          ...
          <div className="ml-auto flex items-center gap-3">
            {orphaned > 0 && <span className="text-xs" style={{ color: '#e0a0a0' }}>⚠️ {orphaned}명 미배정</span>}
...
function Stepper({ value, min, max, suffix, onChange }: { value: number; min: number; max: number; suffix?: string; onChange: (v: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="w-12 text-center font-display text-lg" style={{ color: '#ead9b8' }}>
        {value}
